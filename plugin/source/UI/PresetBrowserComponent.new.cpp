...
PresetBrowserComponent::PresetBrowserComponent(PresetManager& manager)
    ...
    presetList.getViewport()->setScrollBarsShown(true, false, true, false);
    ...
    saveButton.setTooltip("Save current settings as a new preset");
    
    // Set up preset name editor
    ...
    presetNameEditor.setTooltip("Enter a name for the new preset");
    presetNameEditor.onReturnKey = [this] { savePreset(); };
    
    // Set up category combo
    ...
    categoryCombo.setTooltip("Filter presets by category");
    
    // Set up preset directory and scan for presets
    presetDirectory = presetManager.getPresetDirectory();
    scanPresetDirectory();
...
void PresetBrowserComponent::paintListBoxItem(int rowNumber, juce::Graphics& g,
...
void PresetBrowserComponent::listBoxItemClicked(int row, const juce::MouseEvent& e) {
    if (e.mods.isPopupMenu()) {
        juce::PopupMenu menu;
        menu.addItem("Load", [this, row] { presetList.selectRow(row); loadSelectedPreset(); });
        menu.addItem("Delete", [this, row] {
            auto presetName = presets[static_cast<size_t>(row)].file.getFileNameWithoutExtension();
        ...
        menu.showMenuAsync(juce::PopupMenu::Options()
            .withTargetComponent(&presetList)
            .withTargetScreenArea(presetList.getRowPosition(row, true))
        );
    }
}
...
void PresetBrowserComponent::changeListenerCallback(juce::ChangeBroadcaster* source) {
    if (source == &presetList.getViewport()) {
