...
bool PresetManager::savePreset(const juce::File& fileToSave) {
  ...
  presetJson["global"]["temporalDistribution"] =
  ...
  // Assuming StochasticModel has methods to get these parameters for each
  // voice. This part needs to be adapted based on how StochasticModel stores
  // per-voice data. For simplicity, let's assume there's a fixed number of
  // voices or a way to iterate them. This is a placeholder and needs to match
  // the actual StochasticModel interface. For now, we'll save only the global
  // parameters as a starting point.

  // Example for saving per-voice data if model_ had such a structure:
  /*
  for (int i = 0; i < model_.getNumVoices(); ++i) // Assuming
  model_.getNumVoices() exists
  {
      presetJson["voices"][i]["density"] = model_.getDensity(i); // Assuming
  getDensity(voiceIndex) presetJson["voices"][i]["minDistance"] =
  model_.getMinDistance(i);
...
bool PresetManager::loadPreset(const juce::File& fileToLoad) {
  ...
  try {
    ...
    if (presetJson.contains("global")) {
      ...
      if (globalParams.contains("density"))
        model_.setGlobalDensity(globalParams["density"].get<float>());
      if (globalParams.contains("minDistance"))
        model_.setGlobalMinDistance(globalParams["minDistance"].get<float>());
      if (globalParams.contains("pitchOffset"))
        model_.setGlobalPitchOffset(globalParams["pitchOffset"].get<int>());
      if (globalParams.contains("panOffset"))
        model_.setGlobalPanOffset(globalParams["panOffset"].get<float>());
      if (globalParams.contains("velocityOffset"))
        model_.setGlobalVelocityOffset(
            globalParams["velocityOffset"].get<float>());
      if (globalParams.contains("durationOffset"))
        model_.setGlobalDurationOffset(
            globalParams["durationOffset"].get<float>());
      if (globalParams.contains("temporalDistribution"))
        model_.setGlobalTemporalDistribution(
            static_cast<StochasticModel::TemporalDistribution>(
                globalParams["temporalDistribution"].get<int>()));
      if (globalParams.contains("tempoSync"))
        model_.setGlobalTempoSyncEnabled(globalParams["tempoSync"].get<bool>());
      if (globalParams.contains("numVoices"))
        model_.setGlobalNumVoices(globalParams["numVoices"].get<int>());
      if (globalParams.contains("numGrains"))
        model_.setGlobalNumGrains(globalParams["numGrains"].get<int>());
    }

    // --- Stochastic Models (Per Voice) ---
    // Similar to saving, this needs to be adapted based on StochasticModel's
    // structure. This is a placeholder.
    /*
    if (presetJson.contains("voices"))
    {
        ...
        {
            if (i >= static_cast<unsigned>(model_.getNumVoices())) break; //
    Bounds check

            const auto& voiceParams = voicesJson[i];
            if (voiceParams.contains("density")) model_.setDensity(i,
    voiceParams["density"].get<float>());
            // ... and so on
        }
    }
    */
