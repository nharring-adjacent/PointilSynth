...
DebugUIPanel::DebugUIPanel(StochasticModel* model) : stochasticModel(model) {
  ...
  if (stochasticModel)
    pitchSlider.setValue(static_cast<double>(stochasticModel->getPitch()),
  ...
  if (stochasticModel)
    dispersionSlider.setValue(
        static_cast<double>(stochasticModel->getDispersion()),
        juce::dontSendNotification);
  ...
  if (stochasticModel)
    durationSlider.setValue(
        static_cast<double>(stochasticModel->getAverageDurationMs()),
        juce::dontSendNotification);
  ...
  if (stochasticModel)
    durationVariationSlider.setValue(
        static_cast<double>(stochasticModel->getDurationVariation()),
        juce::dontSendNotification);
  durationVariationSlider.onValueChange = [this] {
    durationVariationSliderChanged();
  };
  ...
  if (stochasticModel)
    panSlider.setValue(static_cast<double>(stochasticModel->getCentralPan()),
                       juce::dontSendNotification);
  ...
  if (stochasticModel)
    panSpreadSlider.setValue(
        static_cast<double>(stochasticModel->getPanSpread()),
        juce::dontSendNotification);
  ...
  if (stochasticModel)
    densitySlider.setValue(
        static_cast<double>(stochasticModel->getGlobalDensity()),
        juce::dontSendNotification);
  ...
  if (stochasticModel)
    temporalDistributionComboBox.setSelectedId(
        static_cast<int>(stochasticModel->getGlobalTemporalDistribution()) + 1,
        juce::dontSendNotification);
  temporalDistributionComboBox.onChange = [this] {
    temporalDistributionChanged();
  };
...
void DebugUIPanel::pitchSliderChanged() {
  if (stochasticModel)
    stochasticModel->setPitchAndDispersion(
        static_cast<float>(pitchSlider.getValue()),
        static_cast<float>(dispersionSlider.getValue()));
}
...
void DebugUIPanel::durationSliderChanged() {
  if (stochasticModel)
    stochasticModel->setDurationAndVariation(
        static_cast<float>(durationSlider.getValue()),
        static_cast<float>(durationVariationSlider.getValue()));
}
...
void DebugUIPanel::panSliderChanged() {
  if (stochasticModel)
    stochasticModel->setPanAndSpread(
        static_cast<float>(panSlider.getValue()),
        static_cast<float>(panSpreadSlider.getValue()));
}
...
void DebugUIPanel::densitySliderChanged() {
  if (stochasticModel)
    stochasticModel->setGlobalDensity(
        static_cast<float>(densitySlider.getValue()));
}
...
void DebugUIPanel::temporalDistributionChanged() {
  if (stochasticModel) {
    ...
    if (selectedId > 0) {
      stochasticModel->setGlobalTemporalDistribution(
          static_cast<StochasticModel::TemporalDistribution>(selectedId - 1));
    }
  }
}
